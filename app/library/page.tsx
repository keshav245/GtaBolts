...
import { getMyOwnedMods, getMyDownloadLog } from '@/lib/queries/library';
import { requireUser } from '@/lib/auth-guards';

export default async function LibraryPage() {
  await requireUser();

  const [ownedMods, downloadLog] = await Promise.all([getMyOwnedMods(), getMyDownloadLog()]);
  const hasPurchases = ownedMods.length > 0;
        ...
        <>
          ...
          {downloadLog.length > 0 && (
            <div>
              ...
              <DownloadHistoryTable entries={downloadLog} />
            </div>
          )}
        </>
      )}
